...
export default function Home() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex flex-col items-center justify-start text-center bg-gradient-to-br from-purple-200 via-pink-200 to-yellow-200 overflow-hidden px-4 md:px-0 pt-32 pb-32"
    ...
    >
      <AnimatePresence>
        <>
          {/* Background Blobs */}
          <Motion.div
            className="absolute w-80 h-80 bg-purple-400 rounded-full opacity-30 top-0 left-10 blur-3xl"
            animate={{ y: [0, 25, 0], x: [0, 15, 0] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            exit={{ opacity: 0 }}
          />
          <Motion.div
            className="absolute w-64 h-64 bg-pink-400 rounded-full opacity-30 top-1/2 left-2/3 blur-2xl"
            animate={{ y: [0, -20, 0], x: [0, -15, 0] }}
            transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
            exit={{ opacity: 0 }}
          />
          <Motion.div
            className="absolute w-96 h-96 bg-yellow-400 rounded-full opacity-20 top-3/4 left-1/4 blur-3xl"
          ...
          >
            <Motion.img
              ...
              alt="Profile"
              className="w-72 h-72 md:w-96 md:h-96 rounded-full border-4 border-gradient-to-r from-purple-500 via-pink-500 to-red-500 shadow-2xl object-cover"
              style={{
          ...
          <Motion.p
            className="text-xl md:text-2xl max-w-3xl md:text-left text-center font-medium bg-clip-text text-transparent bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 z-10 leading-relaxed"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
